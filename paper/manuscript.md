...
## 2. Background and related work  `[TO WRITE: needs the verified lit search]`

Themes to synthesise (each reference to be verified, not generated):
1. Measurement theory and construct validity foundations (constructs, operationalisation, reliability vs validity, convergent/discriminant/structural validity).
2. Measurement validity applied to ML and fairness (Jacobs and Wallach, "Measurement and Fairness", FAccT 2021; construct-validity critiques of ML benchmarks).
3. Beyond-accuracy evaluation of text classification (per-class and error analysis; limits of aggregate metrics).
4. Annotation quality and human label variation (Plank; perspectivism; data cascades, Sambasivan et al.).
5. Taxonomy and ontology design and validation; faceted vs hierarchical classification.
6. LLM-as-judge and LLM-as-classifier / zero-shot text classification.
7. Automated text classification and content analysis in social science (text-as-data: Grimmer and Stewart; Grimmer, Roberts and Stewart; Krippendorff on intercoder reliability).
8. AI auditing and governance of sociotechnical measurement (FAccT lineage; EU AI Act).
Reviewer-flagged additions: label-error auditing (confident learning / test-set label errors); task-definition validity in NLP; prior multi-label / hierarchical proposals for facets and overlapping constructs.
